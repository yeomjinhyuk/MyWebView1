...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    CheckBox chk_agree;
    int val_price,val_delivery,val_pay,selected_price=1500,selected_cnt;
    @Override
    ...
    void sumTotal(){
        selected_cnt=Integer.parseInt(edit_count.getText().toString());
        val_price=selected_price*selected_cnt;
        if(val_price<10000)val_delivery=2500;
        else val_delivery=0;
    ...
    public void onClick(View v) {
        ...
        int cnt= Integer.parseInt(str_count);
        switch (v.getId()){
            case R.id.radio1:
            ...
            case R.id.radio3:
                ...
                break;
            case R.id.btn_minus:
                if(cnt>1)edit_count.setText(Integer.toString(cnt-1));
                else Toast.makeText(this, "주문할 수 있는 최소수량은 1개입니다.", Toast.LENGTH_SHORT).show();
                sumTotal();
                break;
            case R.id.btn_plus:
                edit_count.setText(Integer.toString(cnt+1));
                sumTotal();
                break;
            case R.id.btn_pay:
